// NIP-46 relay round-trip test — CJS to avoid Node 25 ESM issues
...
async function testRoundTrip() {
  ...
  const bunkerSkHex = bytesToHex(bunkerSk);
  const bunkerPk = getPublicKey(bunkerSk);
  console.log('Bunker:', bunkerPk.slice(0, 12));

  const convToBunker = nip44.v2.utils.getConversationKey(clientSkHex, bunkerPk);
  const convToClient = nip44.v2.utils.getConversationKey(bunkerSkHex, clientPk);

  // Client subscribes (post-pairing style)
  ...
  subWs.onmessage = async (ev) => {
    try {
      ...
      console.log(`  EVENT: id=${parsed.id?.slice(0,8)} method=${parsed.method} pending=${pending.has(parsed.id)}`);
  ...
  const result = await new Promise(r => {
    const t = setTimeout(() => { console.log('  ❌ TIMEOUT'); r(null); }, 10000);
...
async function testStalePlusFresh() {
  ...
  await wait(2000);
  console.log('  Stale replay:', log.map(l => `${l.method}(${l.id}) matched=${l.matched}`).join(', '));
  ...
  console.log('  All events:', log.map(l => `${l.method}(${l.id}) matched=${l.matched}`).join(' → '));
